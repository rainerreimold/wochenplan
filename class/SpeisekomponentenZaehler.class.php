...
Datei: SpeisekomponenteZaehler.class.php
...
// es genügt daher nur den Eintrag zu senden oder eben nicht.
  ...
  public function inkrementZaehler( $speisekomponente_id, $speise_id ) 
  {
    ...
    // beide Parameter können nicht null sein
  	...
  	try {
		  ...
		  if  (0 == $skz_Id) {
		  	// Neuanlegen
		  	 $sql = "replace into `speisekomponentezaehler` set speisekomponente_id = ".$speisekomponente_id.", speise_id =".$speise_id."; ";
		  
		  }
		  else {
		    // bestehenden Eintrag updaten 
		     $sql = "update `speisekomponentezaehler`  Set zaehler=zaehler+1 where spkomp_id =".$skz_Id."; ";
		  }
	...
	// beide Parameter können nicht null sein
	public function getId( $speisekomponente_id, $speise_id ) 
	{
		try {
		
			$sql = "SELECT spkomp_id from speisekomponentezaehler where speisekomponente_id= ".$speisekomponente_id." and speise_id = ".$speise_id." ";
