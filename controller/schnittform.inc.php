...
function doAction( $action = '', $id = '', $von=0, $lim=0, $order='asc' ) {
	...
	*/
    ...
    else if ( $action == 'zeigeAlleSchnittformen') {
		...
		$sql = "SELECT Distinct `schnittform_id`,`schnittform_bezeichnung`,`beschreibung`, schnittform_art FROM `schnittform` where 1 order By `schnittform_bezeichnung` asc";
		...
		if (DEBUG) echo "<br>".$sql."<br>";
		  ...
		  $ergebnis = $rueckgabe->fetchAll(PDO::FETCH_ASSOC);
	        ...
	        echo '<tr style="padding:8px;"><th style="font-family: Fira ;color:#ddd">Schnittformen f&uuml;r Lebensmittel</th>';
          ...
          echo "</tr>";
	        ...
	        {
	            $schnittform_id=$inhalt['schnittform_id'];
	            
	            echo "<tr style=\"border:1px dotted black;\">";
				echo "<td style=\"background:lightgrey;a color:orange;width:350px;padding:6px;\" class=\"odd\">";
	            
	            echo "<a href=\"/wochenplan/schnittform/details/$schnittform_id\">".$inhalt['schnittform_bezeichnung']."</a>";
       ...
       try {
		...
		$sql = "Select schnittform_bezeichnung, beschreibung from schnittform where schnittform_id = ".$id;
		...
		if (DEBUG) echo "<br>".$sql."<br>";
		  ...
		  $bezeichnung = $ergebnis[0]['schnittform_bezeichnung'];
		  $beschreibung = $ergebnis[0]['beschreibung'];
 
		 echo '<h1 style="background: orange;
	             padding-left:120px;">Schnittform</h1>';
		 ...
		 echo "<input type=\"hidden\" name=\"schnittform_id\" value=\"".$id."\">";     

         echo "<label>Schnittform: </label><input class=\"textform eyecatch\" type=\"text\" name=\"schnittform\"  value=\"".$bezeichnung."\" required /><br>";
         echo '</fieldset>';
         echo "<br>\n";
   ...
   echo "</script>";
       ...
       try {
		...
		if (DEBUG) echo "<br>".$sql."<br>";
         ...
         echo "<label>Schnittform: </label>";
		 echo "<input class=\"textform eyecatch\" type=\"text\" name=\"schnittform\"  value=\"".$bezeichnung."\" required /><br>";
         echo '</fieldset>';
         echo "<br>\n";
		...
		//echo "<textarea id='editor' name='editor'>".$beschreibung."</textarea>";
         ...
         echo ' <fieldset style="background:#cfcfcf; text-align:right; padding:10px; margin-right:10px;">
             <a href="../zeigeAlleSchnittformen">zur&uuml;ck</a>
            </fieldset>
          </form>
       </div>
       <br>
